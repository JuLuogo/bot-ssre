...
- **单次推送张数受 Workers 子请求上限约束**：免费版 50 次/调用，QQ 每张图约 2 次请求（上传+发送）+ 抓图约 1 次/张，
  所以「张数」超过约 15 会报 `Too many subrequests`。`pipeline` 已做优雅中止（发到上限就停并给一句提示，不再刷屏）。
  要一次推更多：把张数调到 10 左右，或升级 Workers 付费版（1000 次/调用）。Queues 也需付费版。
